...
// =================== NAVIGATION MENU ITEMS ACTIVE ON PAGE SCROLL (NEW) ===================
window.addEventListener("scroll", () => {
    ...
    sections.forEach(current => {
        ...
        let sectionTop = current.offsetTop - 50; // Offset for the sticky header
        let id = current.getAttribute("id");

        if (scrollY > sectionTop && scrollY <= sectionTop + sectionHeight) {
            // Find the link with the href that matches the section's id
            const activeLink = document.querySelector(".nav-items a[href*=" + id + "]");
            
            // Remove 'active' class from all other links first
